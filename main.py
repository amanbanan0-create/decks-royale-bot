...
async def build_deck_image(
    ...
    def style_for_card(card):
        ...
        # Base frame palette by in-game rarity
        if "champion" in rarity:
            frame_outer = (235, 188, 67)
            frame_inner = (255, 232, 145)
            glow = (255, 213, 97)
            gem = (226, 193, 93)
            ...
            frame_outer = (255, 160, 65)
            frame_inner = (255, 232, 180)
            glow = (255, 192, 105)
            gem = (222, 190, 118)
            gem_inner = (255, 238, 198)
            badge_fill = None
            badge_top = None
            badge_text = ""
        elif "epic" in rarity:
            frame_outer = (205, 74, 255)
            frame_inner = (242, 202, 255)
            glow = (231, 126, 255)
            gem = (188, 92, 246)
            ...
            frame_outer = (240, 161, 54)
            frame_inner = (255, 218, 118)
            glow = (255, 187, 82)
            gem = (243, 161, 59)
            gem_inner = (255, 223, 152)
            badge_fill = None
            badge_top = None
            badge_text = ""
        else:  # common and fallback
            frame_outer = (148, 154, 170)
            frame_inner = (230, 234, 242)
            glow = (173, 180, 196)
            gem = (176, 180, 195)
        ...
        # Special states override badge + glow to mimic game highlights
        if special == "hero":
            gem = (244, 185, 56)
            ...
            badge_text = "HERO"
            glow = (255, 204, 84)
            frame_outer = (240, 174, 50)
            frame_inner = (255, 226, 132)
        elif special == "evo":
            gem = (200, 73, 255)
            gem_inner = (243, 205, 255)
            ...
            badge_text = "EVO"
            glow = (230, 127, 255)
            frame_outer = (212, 84, 255)
            frame_inner = (250, 209, 255)
        ...
        return {
            ...
            "frame_fill": (21, 25, 34, 255),
            "art_fill": (14, 18, 26, 255),
        }
    ...
    def draw_game_card_frame(x, y, style):
        """
        Unified card look for every deck slot:
        same portrait size as Minion Giant, with rarity-colored frame.
        """
        slot_x1 = x + 12
        slot_y1 = y + 12
        slot_x2 = x + card_w - 12
        ...
        # Outer glow + shell
        draw.rounded_rectangle(
            (slot_x1 - 4, slot_y1 - 4, slot_x2 + 4, slot_y2 + 4),
            radius=28,
            outline=(*style["glow"], 120),
            width=4,
        )
        draw.rounded_rectangle(
            (slot_x1 + 3, slot_y1 + 5, slot_x2 + 3, slot_y2 + 7),
            radius=26,
            fill=(0, 0, 0, 80),
        )
        draw.rounded_rectangle(
            ...
            radius=26,
            fill=style["frame_outer"],
        )
        draw.rounded_rectangle(
            (slot_x1 + 5, slot_y1 + 5, slot_x2 - 5, slot_y2 - 5),
            radius=22,
            fill=style["frame_inner"],
        )
        draw.rounded_rectangle(
            (slot_x1 + 9, slot_y1 + 9, slot_x2 - 9, slot_y2 - 9),
            radius=18,
            fill=style["art_fill"],
        )

        return (slot_x1 + 10, slot_y1 + 10, slot_x2 - 10, slot_y2 - 10)
    ...
    for idx, card in enumerate(normalized):
        ...
        # Official Clash Royale card asset first:
        # we present the original vertical card image with only a subtle shadow,
        # so the deck looks closer to the in-game / official asset style.
        shadow_box(x + 12, y + 12, x + card_w - 12, y + card_h - 12, radius=24, alpha=82)
        ...
        if icon_bytes:
            try:
                ...
                # Keep the official card shape visible and avoid aggressive cropping.
        ...
        if not pasted:
            # Fallback placeholder still respects rarity tint if official asset is missing.
            art_x1, art_y1, art_x2, art_y2 = draw_game_card_frame(x, y, style)
            draw.rounded_rectangle((art_x1, art_y1, art_x2, art_y2), radius=16, fill=(50, 58, 74, 255))

        elixir = card_elixir(card)
        if elixir is not None:
            draw_elixir_drop(x - 2, y + 6, elixir, scale=0.88)

        if style["badge_fill"] and style["badge_text"]:
            bw = 70 if style["badge_text"] != "CHAMP" else 96
            draw_badge(x + card_w - bw - 4, y + 10, style["badge_text"], style["badge_fill"], style["badge_top"], bw)
