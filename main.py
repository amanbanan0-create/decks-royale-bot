...
async def build_deck_image(
    ...
    def style_for_card(card):
        ...
        if "rare" in rarity:
            ...
            edge = (245, 201, 101)
            edge_inner = (255, 233, 166)
            glow = (255, 220, 120)
            gem = (229, 195, 95)
            gem_inner = (255, 236, 179)
        elif "champion" in rarity:
            frame_kind = "champion"
            edge = (122, 148, 176)
            edge_inner = (231, 204, 107)
            glow = (235, 206, 108)
            gem = (224, 191, 92)
    ...
    def draw_game_card_frame(x, y, style):
        """
        Frame tuned to the supplied card samples:
        - common/rare/epic/legendary: dark body with rarity-colored outer edge
        - evo: neon magenta glow and cap
        - hero/champion: silver body with gold crest
        """
        ...
        cap_h = 22 if kind in {'evo', 'hero', 'champion'} else 0
        body_y1 = slot_y1 + cap_h

        # Back shadow
        draw.rounded_rectangle(
            (slot_x1 + 4, body_y1 + 4, slot_x2 + 4, slot_y2 + 6),
            radius=24,
            fill=(0, 0, 0, 88),
        )

        # Glow only for evo / hero / champion, subtle for others.
        glow_alpha = 115 if kind in {'evo', 'hero', 'champion'} else 55
        glow_expand = 6 if kind in {'evo', 'hero', 'champion'} else 3
        draw.rounded_rectangle(
            (slot_x1 - glow_expand, body_y1 - glow_expand, slot_x2 + glow_expand, slot_y2 + glow_expand),
            radius=26,
            outline=(*style['glow'], glow_alpha),
            width=3,
        )

        # Special top crest / cap.
        if cap_h:
        ...
        # Main shell.
        draw.rounded_rectangle(
            (slot_x1, body_y1, slot_x2, slot_y2),
            radius=24,
            fill=style['edge'],
        )
        draw.rounded_rectangle(
            (slot_x1 + 4, body_y1 + 4, slot_x2 - 4, slot_y2 - 4),
            radius=20,
            fill=style['edge_inner'],
        )
        draw.rounded_rectangle(
            (slot_x1 + 8, body_y1 + 8, slot_x2 - 8, slot_y2 - 8),
            radius=16,
            fill=style['art_fill'],
        )

        # Thin inner dark lip, similar to official cards.
        draw.rounded_rectangle(
            (slot_x1 + 10, body_y1 + 10, slot_x2 - 10, slot_y2 - 10),
            radius=14,
            outline=(34, 38, 50, 255),
            width=2,
        )

        return (slot_x1 + 11, body_y1 + 11, slot_x2 - 11, slot_y2 - 11, body_y1)
