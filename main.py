...
async def build_deck_image(
    ...
    """
    V9 renderer:
    - сильнее приближает визуал к Clash Royale;
    - чистая подача карт без дополнительных рамок-контейнеров;
    - более игровая капля эликсира;
    - улучшенные бейджи EVO / HERO / CHAMP;
    - более аккуратный нижний блок Battle Deck;
    - фон с сезонным / режимным арена-оформлением.
    """
    await ensure_cards_catalog()
    ...
    width = 1080
    height = 1520
    bg = Image.new("RGBA", (width, height), theme["top"])
    ...
    dark = (
        max(0, theme["top"][0] - 28),
        max(0, theme["top"][1] - 20),
        max(0, theme["top"][2] - 10),
    )
    ...
    for y in range(height):
        t = y / max(1, height - 1)
        if t < 0.25:
            col = mix(theme["accent"], theme["top"], t / 0.25)
        elif t < 0.72:
            col = mix(theme["top"], theme["mid"], (t - 0.25) / 0.47)
        else:
            col = mix(theme["mid"], dark, (t - 0.72) / 0.28)
        draw.line((0, y, width, y), fill=(*col, 255))

    # Arena-style glow and decorative circles
    for r, alpha in [(440, 34), (330, 28), (240, 24)]:
        cx, cy = width // 2, 210
    ...
    for i in range(8):
        cx = 70 + i * 135
        cy = 84 + (i % 2) * 18
        draw.ellipse((cx - 28, cy - 28, cx + 28, cy + 28), fill=(255, 255, 255, 44))

    # Soft lower arena stripes
    for i in range(6):
        y = 1128 + i * 44
        x_pad = 40 + (i % 2) * 16
        draw.rounded_rectangle(
            (x_pad, y, width - x_pad, y + 5),
            radius=3,
            fill=(255, 255, 255, 40),
        )

    def shadow_box(x1, y1, x2, y2, radius=22, alpha=70):
        draw.rounded_rectangle(
            (x1 + 4, y1 + 6, x2 + 4, y2 + 8),
            radius=radius,
            fill=(0, 0, 0, alpha),
    ...
    def glossy_panel(x1, y1, x2, y2, fill=(38, 78, 150, 220), outline=(176, 210, 255, 255), radius=24):
        shadow_box(x1, y1, x2, y2, radius=radius, alpha=76)
        draw.rounded_rectangle((x1, y1, x2, y2), radius=radius, fill=fill, outline=outline, width=2)
        draw.rounded_rectangle((x1 + 2, y1 + 2, x2 - 2, y1 + int((y2 - y1) * 0.42)), radius=radius, fill=(255, 255, 255, 22))

    # Header block
    title_text, title_font = fit_text(draw, title, width - 56, size=44, bold=True)
    draw.text((26, 18), title_text, font=title_font, fill=(255, 255, 255, 255))

    if subtitle.strip():
        sub_text, sub_font = fit_text(draw, subtitle.strip(), width - 56, size=24, bold=False)
        draw.text((26, 66), sub_text, font=sub_font, fill=(235, 243, 255, 255))
    ...
    def style_for_card(card):
        ...
        if special == "hero":
            return {
                "gem": (243, 183, 54),
                "gem_inner": (255, 228, 128),
                "badge_fill": (219, 152, 43),
        ...
        if special == "evo":
            return {
                "gem": (199, 72, 255),
                "gem_inner": (242, 200, 255),
                "badge_fill": (132, 60, 191),
                "badge_top": (206, 135, 255),
                "badge_text": "EVO",
            }
        if "champion" in rarity:
            return {
                "gem": (224, 189, 92),
                "gem_inner": (255, 236, 172),
                "badge_fill": (138, 100, 27),
                "badge_top": (227, 189, 91),
                "badge_text": "CHAMP",
            }
        return {
            "gem": (226, 194, 95),
            "gem_inner": (255, 238, 182),
            "badge_fill": None,
            "badge_top": None,
            "badge_text": "",
        }
    ...
    def draw_diamond(cx, top_y, style):
        outer = [(cx, top_y), (cx + 17, top_y + 17), (cx, top_y + 34), (cx - 17, top_y + 17)]
        inner = [(cx, top_y + 5), (cx + 9, top_y + 17), (cx, top_y + 29), (cx - 9, top_y + 17)]
        draw.polygon(outer, fill=style["gem"], outline=(255, 250, 225, 255))
        draw.polygon(inner, fill=style["gem_inner"])

    def draw_badge(x1, y1, label, fill, top_fill, width_box):
        shadow_box(x1, y1, x1 + width_box, y1 + 28, radius=10, alpha=50)
        draw.rounded_rectangle((x1, y1, x1 + width_box, y1 + 28), radius=10, fill=fill)
        draw.rounded_rectangle((x1 + 1, y1 + 1, x1 + width_box - 1, y1 + 12), radius=10, fill=top_fill)
    ...
    def draw_elixir_drop(x, y, value, scale=1.0):
        # Closer to in-game teardrop: pointed top + rounded lower body.
        w = int(44 * scale)
        h = int(58 * scale)
        body_top = y + int(16 * scale)
        body_h = int(38 * scale)
        cx = x + w // 2
        fill = (233, 49, 215, 255)
        shine = (255, 198, 255, 255)
        outline = (255, 221, 255, 255)

        # body
        draw.ellipse((x + 2, body_top, x + w - 2, body_top + body_h), fill=fill, outline=outline, width=max(2, int(2 * scale)))
        # pointed top
        draw.polygon(
            [
                (cx, y),
                (x + w - 8, y + int(18 * scale)),
                (x + 8, y + int(18 * scale)),
            ],
            fill=fill,
            outline=outline,
        ...
        draw.polygon(
            [
                ...
                (x + 3, y + int(26 * scale)),
                (x + 9, y + int(38 * scale)),
                (cx, body_top + body_h + 5),
                (x + w - 9, y + int(38 * scale)),
                (x + w - 3, y + int(26 * scale)),
                (x + w - 7, y + int(16 * scale)),
                (cx, y),
            ],
            fill=fill,
        )
        # highlight
        draw.ellipse((x + 10, body_top + 5, x + 20, body_top + 16), fill=shine)
        ...
        if value != "":
            ...
            tw = box[2] - box[0]
            th = box[3] - box[1]
            draw.text((cx - tw / 2, body_top + body_h / 2 - th / 2 - 1), label, font=font, fill=(255, 255, 255, 255))
    ...
    for idx in range(8):
        ...
        # Clean icon presentation: no extra frame container, only icon and shadow.
        icon_box = (x + 5, y + 18, x + card_w - 5, y + 18 + icon_h)
        shadow_box(*icon_box, radius=22, alpha=74)

        icon_bytes = await get_card_image_bytes(card)
        if icon_bytes:
            try:
                icon = Image.open(BytesIO(icon_bytes)).convert("RGBA")
                icon = ImageOps.contain(icon, (icon_box[2] - icon_box[0], icon_box[3] - icon_box[1]))
                px = icon_box[0] + ((icon_box[2] - icon_box[0]) - icon.width) // 2
                py = icon_box[1] + ((icon_box[3] - icon_box[1]) - icon.height) // 2
                bg.paste(icon, (px, py), icon)
            except Exception:
                logging.exception("Failed to render card art %s", card["name"])
                draw.rounded_rectangle(icon_box, radius=20, fill=(46, 76, 141, 255))
        else:
            draw.rounded_rectangle(icon_box, radius=20, fill=(46, 76, 141, 255))

        # top markers
        draw_diamond(x + card_w // 2, y + 1, style)

        elixir = card_elixir(card)
        if elixir is not None:
            draw_elixir_drop(x + 0, y + 8, elixir, scale=0.98)

        if style["badge_fill"] and style["badge_text"]:
            bw = 68 if style["badge_text"] != "CHAMP" else 90
            draw_badge(x + card_w - bw - 8, y + 12, style["badge_text"], style["badge_fill"], style["badge_top"], bw)

        # Name plate
        name_y1 = y + 278
        name_y2 = name_y1 + label_h
        shadow_box(x + 9, name_y1, x + card_w - 9, name_y2, radius=14, alpha=60)
        draw.rounded_rectangle(
            (x + 9, name_y1, x + card_w - 9, name_y2),
            radius=14,
            fill=(26, 40, 90, 236),
            outline=(150, 188, 255, 120),
            width=1,
        )
        name_text, name_font = fit_text(draw, card["name"], card_w - 26, size=24, bold=True)
        draw.text((x + 16, name_y1 + 11), name_text, font=name_font, fill=(10, 16, 44, 255))
        draw.text((x + 14, name_y1 + 9), name_text, font=name_font, fill=(255, 255, 255, 255))

    # Average elixir block
    avg = average_elixir(normalized)
    panel_y = 936
    glossy_panel(30, panel_y, 210, panel_y + 82, fill=(52, 90, 170, 228), outline=(175, 213, 255, 255), radius=20)
    draw_elixir_drop(42, panel_y + 10, "", scale=0.95)
    avg_text = f"{avg:.1f}" if avg is not None else "—"
    draw.text((104, panel_y + 17), avg_text, font=get_font(42, bold=True), fill=(255, 255, 255, 255))

    # Tower troop block
    tower_label = tower_troop.strip() if tower_troop else "Tower Troop"
    glossy_panel(786, panel_y, 1028, panel_y + 82, fill=(78, 152, 223, 228), outline=(195, 230, 255, 255), radius=20)
    # Shield icon
    sx, sy = 802, panel_y + 17
    shield = [(sx + 20, sy), (sx + 40, sy + 8), (sx + 40, sy + 30), (sx + 20, sy + 44), (sx, sy + 30), (sx, sy + 8)]
    draw.polygon(shield, fill=(255, 221, 94, 255), outline=(255, 246, 205, 255))
    draw.polygon([(sx + 20, sy + 7), (sx + 31, sy + 12), (sx + 31, sy + 26), (sx + 20, sy + 34), (sx + 9, sy + 26), (sx + 9, sy + 12)], fill=(101, 176, 244, 255))
    tt_text, tt_font = fit_text(draw, tower_label, 155, size=18, bold=True)
    draw.text((850, panel_y + 15), tt_text, font=tt_font, fill=(255, 255, 255, 255))
    draw.text((850, panel_y + 43), "seasonal layout", font=get_font(14), fill=(236, 245, 255, 255))

    # Improved Battle Deck panel
    glossy_panel(28, 1040, width - 28, 1456, fill=(33, 72, 140, 198), outline=(166, 205, 255, 245), radius=26)
    draw.text((44, 1062), "Battle Deck", font=get_font(32, bold=True), fill=(255, 255, 255, 255))

    # Decorative small slots
    for i in range(3):
        x1 = 280 + i * 120
        draw.rounded_rectangle((x1, 1076, x1 + 52, 18 + 1076), radius=9, outline=(188, 219, 255, 180), width=2)
    ...
    info_lines = [
        mode_text,
        arena_text,
        "Visual theme auto-updates for mode, challenge and current season.",
    ]
    y_cursor = 1114
    for line in info_lines:
        draw.text((44, y_cursor), line, font=get_font(20), fill=(235, 244, 255, 255))
        y_cursor += 34

    # Mini tower silhouettes for game-like footer flavor
    base_y = 1330
    # left king tower
    draw.rounded_rectangle((76, base_y + 50, 174, base_y + 114), radius=8, fill=(118, 83, 44, 255))
    draw.rounded_rectangle((88, base_y, 162, base_y + 62), radius=8, fill=(208, 167, 82, 255), outline=(255, 233, 164, 180), width=2)
    draw.rectangle((103, base_y - 16, 147, base_y + 12), fill=(87, 114, 184, 255))
    # center princess tower
    draw.rounded_rectangle((232, base_y + 58, 314, base_y + 114), radius=8, fill=(118, 83, 44, 255))
    draw.rounded_rectangle((242, base_y + 16, 304, base_y + 72), radius=8, fill=(200, 160, 80, 255), outline=(255, 233, 164, 180), width=2)
    # right card slot / banner
    glossy_panel(776, 1260, 1018, 1422, fill=(64, 106, 186, 222), outline=(175, 214, 255, 255), radius=22)
    draw.text((800, 1282), "Deck Import", font=get_font(24, bold=True), fill=(255, 255, 255, 255))
    draw.text((800, 1320), "Use the button below", font=get_font(18), fill=(231, 241, 255, 255))
    draw.text((800, 1348), "to open the deck", font=get_font(18), fill=(231, 241, 255, 255))
    draw.text((800, 1376), "directly in Clash Royale", font=get_font(18), fill=(231, 241, 255, 255))
