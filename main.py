...
async def build_deck_image(
    ...
    """
    V8.3 renderer:
    - более заметно приближен к игровому виду;
    - убраны внешние рамки вокруг карт;
    - капля эликсира стала более каплевидной;
    - фон по-прежнему зависит от режима / испытания / сезона.
    """
    await ensure_cards_catalog()
    ...
    # Gradient background
    bottom = (
        max(0, theme["mid"][0] - 20),
        max(0, theme["mid"][1] - 10),
        max(0, theme["mid"][2] - 5),
    )
    for y in range(height):
        ...
        if t < 0.72:
            tt = t / 0.72
            r = int(theme["top"][0] * (1 - tt) + theme["mid"][0] * tt)
            g = int(theme["top"][1] * (1 - tt) + theme["mid"][1] * tt)
            b = int(theme["top"][2] * (1 - tt) + theme["mid"][2] * tt)
        else:
            tt = (t - 0.72) / 0.28
            r = int(theme["mid"][0] * (1 - tt) + bottom[0] * tt)
            g = int(theme["mid"][1] * (1 - tt) + bottom[1] * tt)
            b = int(theme["mid"][2] * (1 - tt) + bottom[2] * tt)
        draw.line((0, y, width, y), fill=(r, g, b, 255))

    # Arena-like decorative shapes
    accent = theme["accent"]
    for i in range(7):
        cx = 90 + i * 150
        cy = 70 + (i % 2) * 15
    ...
    for i in range(6):
        ...
        draw.rounded_rectangle(
            (36 + (i % 2) * 12, y, width - 36 - (i % 2) * 18, y + 4),
            radius=2,
            fill=(255, 255, 255, 36),
        )

    # Header
    title_text, title_font = fit_text(draw, title, width - 60, size=44, bold=True)
    draw.text((28, 20), title_text, font=title_font, fill=(255, 255, 255, 255))

    if subtitle.strip():
        sub_text, sub_font = fit_text(draw, subtitle.strip(), width - 60, size=24, bold=False)
        draw.text((28, 68), sub_text, font=sub_font, fill=(233, 243, 255, 255))
    ...
    def style_for_card(card):
        ...
        if special == "hero":
            return {
                "gem_fill": (244, 186, 57),
                "gem_inner": (255, 227, 132),
                "badge_fill": (208, 143, 37),
                "badge_text": "HERO",
            }
        if special == "evo":
            return {
                "gem_fill": (210, 77, 255),
                "gem_inner": (245, 203, 255),
                "badge_fill": (126, 55, 178),
                "badge_text": "EVO",
            }
        if "champion" in rarity:
            return {
                "gem_fill": (219, 190, 92),
                "gem_inner": (255, 236, 174),
                "badge_fill": (136, 98, 27),
                "badge_text": "CHAMP",
            }
        return {
            "gem_fill": (224, 188, 89),
            "gem_inner": (255, 237, 182),
            "badge_fill": None,
            "badge_text": "",
        }

    def draw_top_diamond(cx, top_y, style):
        diamond = [(cx, top_y), (cx + 16, top_y + 16), (cx, top_y + 32), (cx - 16, top_y + 16)]
        inner = [(cx, top_y + 5), (cx + 9, top_y + 16), (cx, top_y + 27), (cx - 9, top_y + 16)]
        draw.polygon(diamond, fill=style["gem_fill"], outline=(255, 248, 220, 255))
    ...
    def draw_elixir_drop(x, y, value, scale=1.0):
        ...
        bulb_h = int(48 * scale)
        top_h = int(24 * scale)
        outline_w = max(2, int(3 * scale))
        fill = (233, 54, 220, 255)
        outline = (255, 219, 255, 255)

        cx = x + bulb_w // 2
        top = y
        bulb_y = y + top_h - 4

        # more droplet-like silhouette
        draw.polygon(
            [
                (cx, top),
                (x + bulb_w - 6, bulb_y + 8),
                (x + bulb_w - 2, bulb_y + bulb_h // 2),
                (cx, bulb_y + bulb_h + 6),
                (x + 2, bulb_y + bulb_h // 2),
                (x + 6, bulb_y + 8),
            ],
        ...
        draw.ellipse(
            (x + 2, bulb_y, x + bulb_w - 2, bulb_y + bulb_h),
            fill=fill,
            outline=outline,
            width=outline_w,
        )
        ...
        if value != "":
            ...
            th = box[3] - box[1]
            draw.text(
                (cx - tw / 2, bulb_y + bulb_h / 2 - th / 2 - 2),
                label,
                font=font,
                fill=(255, 255, 255, 255),
            )

    def draw_shadow_box(x1, y1, x2, y2, radius=20):
        draw.rounded_rectangle(
            (x1 + 4, y1 + 6, x2 + 4, y2 + 8),
            radius=radius,
            fill=(0, 0, 0, 66),
    ...
    for idx in range(8):
        ...
        y = start_y + row * (card_total_h + gap_y)

        style = style_for_card(card)

        # Frameless card presentation: only art + soft shadow.
        art_x1 = x + 6
        art_y1 = y + 16
        art_x2 = x + card_w - 6
        art_y2 = y + 16 + art_h

        draw_shadow_box(art_x1, art_y1, art_x2, art_y2, radius=22)
        ...
        if icon_bytes:
            ...
            # fallback silhouette
            draw.rounded_rectangle((art_x1, art_y1, art_x2, art_y2), radius=18, fill=(52, 80, 145, 255))

        # top diamond + elixir
        draw_top_diamond(x + card_w // 2, y + 2, style)
        elixir = card_elixir(card)
        if elixir is not None:
            draw_elixir_drop(x + 2, y + 8, elixir, scale=0.95)

        # optional badge
        if style["badge_fill"] and style["badge_text"]:
            bw = 72 if style["badge_text"] != "CHAMP" else 96
            rounded_badge(
                draw,
                (x + card_w - bw - 8, y + 12, x + card_w - 8, y + 42),
                style["badge_text"],
        ...
        # name plate only, no outer card frame
        label_y1 = y + art_h + 26
        label_y2 = label_y1 + 50
        draw_shadow_box(x + 10, label_y1, x + card_w - 10, label_y2, radius=14)
        draw.rounded_rectangle(
            (x + 10, label_y1, x + card_w - 10, label_y2),
            radius=14,
            fill=(29, 42, 92, 228),
            outline=(146, 184, 255, 120),
            width=1,
        )
        name_text, name_font = fit_text(draw, card["name"], card_w - 26, size=25, bold=True)
        draw.text((x + 17, label_y1 + 12), name_text, font=name_font, fill=(10, 18, 44, 255))
        draw.text((x + 15, label_y1 + 10), name_text, font=name_font, fill=(255, 255, 255, 255))

    # Bottom panels
    avg = average_elixir(normalized)
    panel_y = 935

    draw.rounded_rectangle((30, panel_y, 210, panel_y + 82), radius=20, fill=(48, 87, 168, 220))
    draw.rounded_rectangle((30, panel_y, 210, panel_y + 82), radius=20, outline=(169, 210, 255, 255), width=2)
    draw_elixir_drop(42, panel_y + 12, "", scale=0.95)
    avg_text = f"{avg:.1f}" if avg is not None else "—"
    avg_font = get_font(42, bold=True)
    draw.text((102, panel_y + 19), avg_text, font=avg_font, fill=(255, 255, 255, 255))

    tower_label = tower_troop.strip() if tower_troop else "Tower Troop"
    draw.rounded_rectangle((780, panel_y, 1024, panel_y + 82), radius=20, fill=(73, 152, 224, 220))
    draw.rounded_rectangle((780, panel_y, 1024, panel_y + 82), radius=20, outline=(193, 229, 255, 255), width=2)
    tt_text, tt_font = fit_text(draw, tower_label, 205, size=18, bold=True)
    draw.text((796, panel_y + 17), tt_text, font=tt_font, fill=(255, 255, 255, 255))
    draw.text((796, panel_y + 45), "season-aware deck theme", font=get_font(14), fill=(236, 245, 255, 255))

    info_y = 1052
    draw.text((42, info_y), "Battle Deck", font=get_font(30, bold=True), fill=(255, 255, 255, 255))
    draw.text((42, info_y + 42), f"Mode: {deck_mode_badge(mode_label, source_label)}", font=get_font(20), fill=(236, 244, 255, 255))
    arena_line = f"Arena theme: {theme['arena_name']}"
    if season_id:
        arena_line += f" · Season {season_id}"
    draw.text((42, info_y + 72), arena_line, font=get_font(20), fill=(236, 244, 255, 255))
    draw.text((42, info_y + 102), "Background auto-changes by mode/challenge and season.", font=get_font(18), fill=(219, 233, 255, 255))
