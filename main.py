...
async def build_deck_image(
    ...
    for idx, card in enumerate(normalized):
        ...
        art_x1, art_y1, art_x2, art_y2, body_y1 = draw_game_card_frame(x, y, style)
        ...
        if icon_bytes:
            try:
                ...
                target_w = max(1, art_x2 - art_x1)
                target_h = max(1, art_y2 - art_y1)

                # Reference-based fit: crop slightly so the portrait fills the card like the samples.
                icon = ImageOps.fit(
                    icon,
                    (target_w, target_h),
                ...
                bg.paste(icon, (art_x1, art_y1), icon)
                pasted = True
            except Exception:
                logging.exception("Failed to render card art %s", card["name"])

        if not pasted:
            draw.rounded_rectangle((art_x1, art_y1, art_x2, art_y2), radius=14, fill=(50, 58, 74, 255))

        # Top gem and badges on special cards, matching user references.
        if style['frame_kind'] in {'evo', 'hero', 'champion'}:
            draw_diamond(x + card_w // 2, y + 2, style)

        elixir = card_elixir(card)
        if elixir is not None:
            draw_elixir_drop(x - 2, body_y1 + 2, elixir, scale=0.84)

        if style["badge_fill"] and style["badge_text"]:
            bw = 70 if style["badge_text"] != "CHAMP" else 96
            badge_x = x + card_w - bw - 6
            badge_y = body_y1 + 8
            draw_badge(badge_x, badge_y, style["badge_text"], style["badge_fill"], style["badge_top"], bw)
