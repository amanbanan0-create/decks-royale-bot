...
async def build_deck_image(
    ...
    for idx, card in enumerate(normalized):
        ...
        # Minimal invisible slot only for alignment; the user asked for a frameless look.
        slot_x1 = x + 2
        slot_y1 = y + 4
        slot_x2 = x + card_w - 2
        slot_y2 = y + card_h - 4
        ...
        if icon_bytes:
            try:
                ...
                # Preserve the original in-game frame and fit the whole card into the slot.
                ...
                # Very subtle shadow to keep the cards readable on dark backgrounds.
                shadow_box(px + 8, py + 10, px + icon.width - 8, py + icon.height - 4, radius=18, alpha=58)
        ...
        if not pasted:
            draw.rounded_rectangle((slot_x1 + 16, slot_y1 + 16, slot_x2 - 16, slot_y2 - 16), radius=18, fill=(45, 52, 68, 255), outline=(96, 108, 134, 255), width=2)

        # Keep only the elixir indicator. Remove small EVO/HERO labels for a cleaner layout.
        elixir = card_elixir(card)
        if elixir is not None:
            draw_elixir_drop(x - 2, y + 4, elixir, scale=0.90)
