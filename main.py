...
async def build_deck_image(
    ...
    # Layout of 8 official card assets on dark background
    cols = 4
    card_w = 232
    card_h = 274
    gap_x = 2
    ...
    def style_for_card(card):
        ...
        # Default in-game frame language, tuned using the reference samples.
        frame_kind = "common"
        edge = (201, 206, 218)
        edge_inner = (82, 90, 108)
        glow = (170, 176, 190)
        gem = (187, 191, 204)
        gem_inner = (240, 242, 247)
        accent = (40, 45, 60)
        badge_fill = None
        badge_top = None
        badge_text = ""
        ...
        if "rare" in rarity:
            frame_kind = "rare"
            edge = (255, 128, 40)
            edge_inner = (255, 192, 92)
            glow = (255, 145, 62)
            gem = (242, 174, 70)
            gem_inner = (255, 223, 140)
            accent = (52, 39, 28)
        elif "epic" in rarity:
            frame_kind = "epic"
            edge = (193, 84, 255)
            edge_inner = (237, 188, 255)
            glow = (223, 122, 255)
            gem = (189, 89, 247)
            gem_inner = (242, 202, 255)
            accent = (48, 32, 72)
        elif "legendary" in rarity:
            # Legendary uses a beveled diamond-like metallic frame with a purple accent.
            frame_kind = "legendary"
            edge = (120, 126, 141)
            edge_inner = (228, 232, 239)
            glow = (204, 126, 255)
            gem = (205, 149, 255)
            gem_inner = (245, 222, 255)
            accent = (127, 79, 198)
        elif "champion" in rarity:
            frame_kind = "champion"
            edge = (116, 143, 174)
            edge_inner = (236, 206, 105)
            glow = (235, 206, 108)
            gem = (224, 191, 92)
            gem_inner = (255, 235, 170)
            accent = (58, 82, 113)
            badge_fill = (135, 98, 30)
            badge_top = (230, 194, 93)
            badge_text = "CHAMP"

        # Special states from user samples.
        if special == 'hero':
            frame_kind = 'hero'
            edge = (116, 143, 174)
            edge_inner = (236, 206, 105)
            glow = (237, 208, 107)
            gem = (222, 188, 85)
            gem_inner = (255, 236, 171)
            accent = (58, 82, 113)
            badge_fill = (212, 152, 44)
            badge_top = (255, 214, 108)
            badge_text = 'HERO'
        elif special == 'evo':
            frame_kind = 'evo'
            edge = (216, 84, 255)
            edge_inner = (249, 208, 255)
            glow = (244, 113, 255)
            gem = (197, 82, 247)
            gem_inner = (247, 214, 255)
            accent = (92, 50, 140)
        ...
        return {
            ...
            'accent': accent,
            'frame_fill': (31, 36, 49, 255),
            'art_fill': (12, 16, 22, 255),
        }
    ...
    def draw_game_card_frame(x, y, style):
        """
        Frame tuned to the user references.
        Legendary cards use a beveled diamond/octagon frame.
        Champion/hero use a more angular crowned frame.
        Inner padding is reduced so the art sits closer to the frame.
        """
        kind = style['frame_kind']
        ...
        def octagon(x1, y1, x2, y2, cut=14):
            return [
                (x1 + cut, y1),
                (x2 - cut, y1),
                (x2, y1 + cut),
        ...
        def shield(x1, y1, x2, y2, cut=14, shoulder=8):
            return [
                (x1 + cut, y1),
                (x2 - cut, y1),
                (x2, y1 + cut + shoulder),
        ...
        cap_h = 24 if kind in {'evo', 'hero', 'champion'} else 0
        body_y1 = slot_y1 + cap_h
        special_frame = kind in {'evo', 'hero', 'champion'}
        legendary_frame = kind == 'legendary'

        # Shadow
        shadow_pts = (shield if special_frame else octagon)(slot_x1 + 4, body_y1 + 4, slot_x2 + 4, slot_y2 + 6, cut=14)
        draw.polygon(shadow_pts, fill=(0, 0, 0, 86))

        # Glow
        glow_alpha = 120 if special_frame else (80 if legendary_frame else 55)
        glow_expand = 5 if special_frame else (4 if legendary_frame else 2)
        glow_pts = (shield if special_frame else octagon)(slot_x1 - glow_expand, body_y1 - glow_expand, slot_x2 + glow_expand, slot_y2 + glow_expand, cut=16)
        draw.line(glow_pts + [glow_pts[0]], fill=(*style['glow'], glow_alpha), width=3, joint='curve')

        # Top crest / crown section for evo, hero, champion.
        if special_frame:
            cx = (slot_x1 + slot_x2) // 2
            top = [
                (cx - 36, body_y1 + 2),
                (cx - 30, slot_y1 + 10),
                (cx - 18, slot_y1 + 2),
            ...
            draw.polygon(top, fill=style['edge'])
            inner_top = [
                (cx - 27, body_y1 + 3),
                (cx - 22, slot_y1 + 11),
                (cx - 13, slot_y1 + 7),
            ...
            draw.polygon(inner_top, fill=style['edge_inner'])

        # Main shell
        poly_func = shield if special_frame else octagon
        outer = poly_func(slot_x1, body_y1, slot_x2, slot_y2, cut=14)
        inner = poly_func(slot_x1 + 4, body_y1 + 4, slot_x2 - 4, slot_y2 - 4, cut=12)
        accent = poly_func(slot_x1 + 6, body_y1 + 6, slot_x2 - 6, slot_y2 - 6, cut=11)
        art = poly_func(slot_x1 + 8, body_y1 + 8, slot_x2 - 8, slot_y2 - 8, cut=9)

        draw.polygon(outer, fill=style['edge'])
        draw.polygon(inner, fill=style['edge_inner'])

        # Accent line: purple for legendary, darker lip for others.
        draw.polygon(accent, fill=style['accent'])
        draw.polygon(art, fill=style['art_fill'])
        draw.line(art + [art[0]], fill=(34, 38, 50, 255), width=2, joint='curve')

        return (slot_x1 + 9, body_y1 + 9, slot_x2 - 9, slot_y2 - 9, body_y1)
    ...
    def trim_card_background(icon: Image.Image) -> Image.Image:
        """Remove flat white/gray card backplates if an asset comes with one."""
        icon = icon.convert("RGBA")

        # First, crop by existing alpha if present.
        ...
        samples = [
            ...
            icon.getpixel((w - 2, 1)),
            icon.getpixel((1, h - 2)),
            icon.getpixel((w - 2, h - 2)),
            icon.getpixel((w // 2, 1)),
            icon.getpixel((w // 2, h - 2)),
            icon.getpixel((1, h // 2)),
            icon.getpixel((w - 2, h // 2)),
        ]
        opaque = [p for p in samples if len(p) == 4 and p[3] > 200]
        if len(opaque) < 4:
            return icon

        avg = tuple(sum(p[i] for p in opaque) // len(opaque) for i in range(3))
        spread = max(abs(p[i] - avg[i]) for p in opaque for i in range(3))

        # Only key out very consistent light gray / white backgrounds.
        if spread > 18 or sum(avg) / 3 < 150:
            return icon
    ...
    for idx, card in enumerate(normalized):
        ...
        # Minimal slot only for alignment on the dark canvas.
        slot_x1 = x
        slot_y1 = y + 2
        slot_x2 = x + card_w
        slot_y2 = y + card_h
        ...
        if icon_bytes:
            try:
                ...
                icon = trim_card_background(icon)
                target_w = max(1, slot_x2 - slot_x1)
                target_h = max(1, slot_y2 - slot_y1)

                # Fill the slot more aggressively now that the gray backplate is removed.
                icon = ImageOps.contain(
                    icon,
                    (target_w, target_h),
                    method=Image.Resampling.LANCZOS,
                )
                px = slot_x1 + (target_w - icon.width) // 2
                py = slot_y1 + (target_h - icon.height) // 2

                shadow_box(px + 10, py + 12, px + icon.width - 10, py + icon.height - 6, radius=18, alpha=54)
        ...
        if not pasted:
            draw.rounded_rectangle((slot_x1 + 22, slot_y1 + 22, slot_x2 - 22, slot_y2 - 22), radius=18, fill=(45, 52, 68, 255), outline=(96, 108, 134, 255), width=2)

        elixir = card_elixir(card)
        if elixir is not None:
            draw_elixir_drop(x - 4, y + 2, elixir, scale=0.92)
