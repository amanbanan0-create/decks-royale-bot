...
async def get_card_image_bytes(raw_card) -> bytes | None:
    ...
    if not candidates:
        logging.warning("No card icon URL available for %s", card_name)
        return None

    # Сначала проверяем положительный кэш.
    ...
    async with image_cache_lock:
        # Пока ждали lock, другая задача могла уже загрузить иконку.
        for url in candidates:
            cached = card_image_cache.get(url)
            if cached:
                return cached
        ...
        async with httpx.AsyncClient(
            ...
            for index, url in enumerate(candidates, 1):
                if url in card_image_bad_urls:
                    continue

                try:
                    response = await client.get(url)

                    # 404 у специальных EVO/HERO ассетов — ожидаемый случай:
                    # помечаем URL плохим и тихо переходим к следующему.
                    if response.status_code == 404:
                        card_image_bad_urls.add(url)
                        logging.warning(
                            "Card icon 404 for %s; trying fallback %s/%s",
                            card_name,
                            index,
                            len(candidates),
                        )
                    ...
                    if "image" not in content_type.lower():
                        card_image_bad_urls.add(url)
                        logging.warning(
                            "Card icon for %s returned non-image content; trying fallback",
                            card_name,
                        )
                        continue
                    ...
                    if not data:
                        card_image_bad_urls.add(url)
                        logging.warning(
                            "Card icon for %s returned empty body; trying fallback",
                            card_name,
                        )
                        continue
                    ...
                    if index > 1:
                        logging.info(
                            "Card icon fallback worked for %s (candidate %s/%s)",
                            card_name,
                            index,
                            len(candidates),
                        )
                    ...
                    logging.warning(
                        "Card icon request failed for %s: HTTP %s; trying fallback",
                        card_name,
                        status,
                    )
                    ...
                    logging.warning(
                        "Card icon network error for %s: %s; trying fallback",
                        card_name,
                        type(exc).__name__,
                    )
                    ...
                    logging.warning(
                        "Card icon fallback error for %s: %s",
                        card_name,
                        type(exc).__name__,
                    )

    logging.warning(
        "All card icon candidates failed for %s; using placeholder",
        card_name,
    )
    return None
...
# URL, которые уже вернули 404/невалидный контент. Не запрашиваем их повторно.
card_image_bad_urls: set[str] = set()
