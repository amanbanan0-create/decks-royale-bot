...
async def fetch_top_100_players() -> list[dict]:
    """
    Получает текущий глобальный leaderboard.
    Сначала пробуем Ranked / Path of Legend, затем Trophy Road.
    """
    ...
    for endpoint in endpoints:
        try:
            ...
            # Официальный API обычно возвращает {"items": [...]},
            # но обрабатываем и другие формы ответа, чтобы прокси
            # не ломал бота при небольшом отличии JSON.
            items = []
            ...
            if isinstance(data, list):
                ...
                if isinstance(data.get("items"), list):
                    ...
                    if isinstance(nested.get("items"), list):
                        items = nested["items"]
                    elif isinstance(nested.get("players"), list):
                        items = nested["players"]

            logging.info(
                "Leaderboard %s: response_type=%s, players=%s, keys=%s",
                endpoint,
                type(data).__name__,
                len(items),
            ...
            if isinstance(data, dict):
                diagnostics.append(
                    f"{endpoint}: пусто; keys={list(data.keys())}"
                )
            else:
                diagnostics.append(
                    f"{endpoint}: пусто; type={type(data).__name__}"
                )

        except Exception as exc:
            logging.exception("Leaderboard endpoint %s failed", endpoint)
            diagnostics.append(f"{endpoint}: {exc}")

    detail = " | ".join(diagnostics) if diagnostics else "нет диагностики"
    raise RuntimeError(
        "Не удалось получить мировой топ игроков. " + detail
    )
...
async def api_test():
    """
    Безопасная диагностика: ключ не выводится.
    Показывает только форму ответа leaderboard.
    """
    if not api_is_ready():
        return {
            "ok": False,
